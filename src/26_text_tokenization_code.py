...
"""
- 이번 글은 컴퓨터가 인간의 언어를 이해하는 자연어 처리를 위한 첫번째 글 입니다.
    자연어 처리는 인공지능을 구현하기 위해 반드시 넘어야할 산이며, 굉장히 넘기 힘든 난제이기도 합니다.
    언어라는 것 자체만으로도 학문적으로 얼마나 깊게 들어갈 수 있는지 모를 정도인데,
    이것을 인공적으로 구현해야만 하기 때문입니다.
    최근에는 GPT 모델이 이를 정복할 수 있다는 희망을 보여주며, 
    자연어 처리 영역을 넘어서 강인공지능의 탄생까지 단숨에 도약하려는 움직임을 보이고 있습니다.
    즉, 현 시점 자연어처리가 데이터 분석이나 인공지능 분야에서 가장 중요한 주제라고 할 수 있습니다.
