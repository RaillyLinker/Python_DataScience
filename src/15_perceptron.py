...
- 인공 신경망 발전사
    1943년, Warren McCulloch-Walter Pitts 의 논문에 신경을 그물망 형태로 연결하면 사람의 뇌처럼 동작할 수 있다는 가능성을 주장합니다.
        해당 논문에서는 Warren McCulloch-Walter Pitts 와 Walter Pitts 가 처음으로 간소화된 뉴런 개념을 발표하였으며,
        이를 McCulloch-Pitts(MCP) Neuron 이라고 합니다.
        MCP Neuron 의 구조는 생명체의 신경 세포의 구조를 기반으로 하였으며,
        신경세포가 여러개의 수상돌기를 통하여 외부 자극을 수용하고, 축삭 돌기를 통해 신호가 전달되며, 
        축삭끝 가지들을 통해 출력 신호를 다음 신경세포로 전달하는 과정을 간단한 논리회로로 표현 하였습니다.
        이전 시대에는 인공적으로 사람의 정신과 지능과 같이 추상적이고 고차원적인 개념을 구현하기 위해 무엇이 필요한지에 대해 방향도 잡지 못하던 상황이었다면,
        사람의 뇌 신경, 뇌 구조 자체를 복제하는 방식으로 인공 지능을 구현 가능할 것이라는 방향성을 잡아준 것으로 의의가 있습니다.
        
        추가하자면, 복잡하고 추상적인 개념을 더 단순한 요소로 명확하게 정의할 수 있다는 믿음, 
        예를들어 생명체의 정신 활동을 불가침한 종교적 성역이 아닌, 
        물리적, 화학적으로 설명 가능하다는 이와 같은 주장을 환원주의라고 부릅니다.
        
    1957년, 미국의 신경 생물학자 Frank Rosenblatt 가 MCP Neuron 모델을 기반으로 퍼셉트론 학습 개념을 처음 발표하였습니다.
        이것이 바로 로젠블라트 퍼셉트론으로, 입력값과 정답을 통해 선형 회귀 모델의 가중치를 학습 시킬 수 있는 최초의 모델입니다.
        로젠블라트 퍼셉트론과 MCP Neuron 의 가장 큰 차이점은 학습 알고리즘의 유무인데,
        로젠블라트가 제시한 초기 퍼셉트론 학습 규칙은 아래와 같이 매우 간단합니다.
        1. 뉴런 모델의 가중치를 0 또는 랜덤한 작은 값으로 초기화
        2. 각 훈련 샘플들에 대하여 출력값을 계산
        3. 가중치를 w, 가중치 변경값을 dw, 정답값을 y, 예측값을 y`, 변수를 x, 학습률을 r 이라고 했을 때,
            w = w + dw
            dw = r * (y - y`) * x
        초기 퍼셉트론 학습 방식은 위와 같은데, 학습에 사용되는 수식을 본다면, 
        학습이 잘 이루어져 y - y` 가 0 이 된다면 학습이 멈추게 되므로 이치에 맞는 방식이라는 것을 알 수 있습니다.
        로젠블라트 퍼셉트론의 구조적 특징으로는 활성화 함수에 계단 함수를 사용한다는 것으로,
        일정 이하의 자극을 차단하는 생물 신경의 역치와 같이 임계점 아래의 값은 0, 임계점 이상의 값은 1로 반환합니다.
        
    1960년, Bernard Widrow 와 그의 박사과정 학생이던 Tedd Hoff 가 
        로젠블라트의 한계를 극복하기 위해 ADALINE(ADAptive LInear NEuron - 적응형 선형 뉴런) 퍼셉트론을 개발하였습니다.
        로젠블라트 퍼셉트론과 ADALINE 의 구조적 차이점으로는, 활성화 함수에 계단 함수 대신 선형 활성화 함수를 사용한다는 것입니다.
        (ADALINE 에서 사용한 선형 활성화 함수는, 입력값을 그대로 반환하는 항등함수(Identity Function)를 사용하였습니다.)
        
        그조를 보자면, 로젠 블라트 퍼셉트론은 입력 -> 계단함수 -> 오차 피드백, 출력 이라면,
        ADALINE 은 입력 -> 선형 활성화 함수 -> 오차 피드백 -> 계단함수 -> 출력 입니다.
        
        여전히 출력 결과값은 계단 함수가 반환하는 것을 볼 수 있지만, 파라미터 학습시 사용되는 되는 값은 선형함수의 결과값을 사용하는 차이점이 있습니다.
        이러한 구조의 장점은, 로젠블라트에서 계단식 출력값을 파라미터 학습용 오차로 사용하였을 때에는 임계값에 따라 비선형적으로 오차값이 변화하기에 학습이 들쑥날쭉하지만,
        ADALINE 구조는 학습에 사용되는 값이 연속형 데이터이므로 학습이 완만하고 안정적이게 이뤄진다는 것입니다.
        
        또한 ADALINE 이 로젠 블라트에 비해 개선된 부분은 구조뿐 아닙니다.
        
        ADALINE 퍼셉트론 파라미터 학습 방식에는 경사 하강법(Gradient Descent)을 도입하였습니다.
        
        경사하강법은, 퍼셉트론 예측값과 정답값 사이의 차이값을 줄여나가는 방식으로 서서히 파라미터를 수정해나가는 방식입니다.
        로젠블라트 퍼셉트론 학습 역시 오차를 줄이는 방식으로 파라미터를 수정하지만 경사 하강법이 이와 다른점으로는,
        오차값을 구할 때 사용하는 값이 연속성을 띄게 되었므로 이에 대한 미분 값을 구하여 사용한 것입니다.
        미분으로 구한 오차값의 기울기를 기반으로 오차값이 작아지는 방향으로 합리적인 변동폭으로 파라미터를 수정해나가는 이 방식은,
        마치 구덩이의 아래쪽으로 미끄러지듯이 오차가 작아지기 시작하다가, 경사가 0이 되는 지점에서 완전히 멈추게 되는 형태를 볼 수 있습니다.
        이렇게 오차가 경사를 타고 아래로 미끄러져 나가는 모양을 본따(영어로는, 미분 기울기를 0 을 향해 줄여나가는 것을 의미하여) 이 학습 방식을 경사 하강법이라 부르는 것입니다.
        
        이 경사 하강법은 현재까지 사용되는 딥러닝 학습 방식의 주요 원리이므로 정말 획기적이고 중요한 개념입니다.
        
        위와 같이 로젠블라트 퍼셉트론을 기반으로 여러 획기적인 개선점을 내놓았던 아델라인은 
        학습 가능한 선형 회귀 모델로써 그 성능을 입증한 이후 딥러닝, 서포트 벡터 머신 등 머신러닝의 중요한 알고리즘의 개발에 영향을 주었습니다.
...
- Xor 문제
    ...
    이는, 퍼셉트론이 가장 근본적인 논리구조의 하나를 해결하지 못한다는 뜻이 되며,
    퍼셉트론이 선형적으로 구분되는 데이터만을 분류할 수 있다는 것을 증명한 것이 됩니다.
"""
# (단층 퍼셉트론으로 논리 회로 구현)
...
- 잠시 이해가 필요한 배경 지식을 설명합니다.
    경사 하강법은 경사를 타고 내려가다가 더이상 내려갈 수 없이 기울기가 0이 되는 구간을 찾는 방식이라고 설명했습니다.
    그런데, 기울기가 0이 되는 구간은 하나 이상일 수 있습니다.
    무슨 의미냐면, 실제 오차의 최소값은 10 이라는 위치에 있는데, 경사를 내려가는 도중 오차가 50인 위치에서 기울기가 0이 될 수 있다는 것입니다.
    이렇게 되면 오차 10까지 내려가야 최적의 결과를 낼 수 있는데, 경사 하강법은 오차 50일 때에 이것이 정답이라고 판단하게 됩니다.
    이러한 현상을 지역 최소값이라고 부르며, 이렇게 기울기가 상실되는 현상을 기울기 소실(Vanishing Gradient) 이라고 합니다.
    아래 설명할 경사 하강법의 종류와 발전은 이러한 기울기 소실 문제를 해결하는 노력이 포함됩니다.
    
(확률적 경사 하강법(Stochastic Gradient Descent, SGD))
- 일반 경사 하강법(Gradient Descent, GD) 를 개선한 첫번째 경사 하강법입니다.
    일반 경사 하강법은, 전체 훈련 데이터셋에서 계산한 그레디언트의 반대 방향으로 한걸음씩 진행하며 비용함수를 최소화하는 방법인데,
    배치 경사 하강법이라고도 불립니다.
    즉, 데이터셋의 모든 예측 결과가 나와야만 한번의 경사 하강이 이뤄지는 것인데, 
    이렇게 된다면 한 배치당 데이터 개수가 많아질수록 계산 비용이 매우 커질 수 있습니다.
    SGD 는 이렇게 전체 데이터의 기울기를 구하여 가중치를 업데이트 하는 것이 아니라, 각 훈련샘플에 대해 기울기를 구하여 학습을 하는 것을 의미합니다.
    SGD 는 배치 경사하강법보다 오차의 궤적이 어지러울 수 밖에 없는데,
    이로 인하여 생기는 단점으로는 학습 결과가 보다 랜덤성이 짙어진다는 것이고,
    장점으로는, 오히려 오차 궤적이 요동치기에 지역 최소값을 탈출할 가능성이 높아지며, 계산 비용도 줄어들고, 그렇기에 학습 속도도 더 빨라질 수 있습니다.
    게다가 하나의 데이터에 하나의 학습이 이루어지므로, 새로운 훈련 데이터가 도착하는대로 새로운 학습을 진행할 수 있는 온라인 학습(Online Learning) 도 가능해집니다.
    SGD 를 잘 사용하기 위해서는, 훈련 샘플을 무작위로 섞는 것이 중요하며, 에포크마다도 훈련 샘플의 순서를 섞어주는 것이 좋습니다.
    
(미니 배치 경사 하강법(Mini-Batch Gradient Descent))
- 배치 경사 하강법과 SGD 를 절충한 방식으로,
    배치 크기를 부담이 되지 않는 작은 숫자로 줄여서 경사하강법을 수행하면,
    배치 경사 하강법과 SGD 의 장점을 동시에 취할 수 있다는 발상으로 만들어진 방식입니다.
