...
- 인공 신경망 발전사
    ...
    1960년, Bernard Widrow 와 그의 박사과정 학생이던 Tedd Hoff 가 
        ...
        ADALINE 퍼셉트론 파라미터 학습 방식에는 경사 하강법을 도입하였습니다.
        
        경사하강법은, 퍼셉트론 예측값과 정답값 사이의 차이값을 줄여나가는 방식으로 서서히 파라미터를 수정해나가는 방식으로,
        앞서 구조적으로 계단 함수가 아닌 선형 함수를 사용하였을 때의 장점을 언급했듯,
        오차값 역시 연속성을 띄게 되므로 이에 대한 미분이 가능해지게 되는 장점이 생기는 것입니다.
        여기서 구한 기울기를 기반으로 오차값이 작아지는 방향으로 파라미터를 수정해나간다면,
        마치 구덩이의 아래쪽으로 미끄러지듯이 오차가 작아지기 시작하다가, 경사가 0이 되는 지점에서 완전히 멈추게 되는 형태를 볼 수 있습니다.
        이렇게 오차가 경사를 타고 아래로 미끄러져 나가는 모양을 일컬어 경사 하강법이라 부르는 것입니다.
