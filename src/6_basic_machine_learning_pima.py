...
# 모델 검증
accuracies = cross_val_score(estimator=classifier, X=X_test, y=y_test, cv=StratifiedKFold(n_splits=10, shuffle=True))
...
classifiers = {
    ...
    'Voting':
        VotingClassifier(estimators=[('lr', AdaBoostClassifier(algorithm='SAMME')), ('rf', RandomForestClassifier()),
                                     ('gnb', SVC(kernel="linear"))]),
    'Stacking':
        StackingClassifier(estimators=[('lr', AdaBoostClassifier(algorithm='SAMME')), ('rf', RandomForestClassifier()),
