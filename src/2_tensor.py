...
- 텐서란 무엇일까요?
    ...
    1 이나 2 와 같이 독립된 데이터가 하나 존재하면 이를 Scala라고 부릅니다.

    다음으로, 이러한 스칼라 값이 모여서 [1, 2, 3] 과 같이 리스트가 된 것을 Vector 라고 부릅니다.
    Vector 는 순서가 존재하는 Scala 의 묶음이죠.

    이러한 Vector 가 묶이면 뭐라고 할까요?
    [[1, 2, 3], [1, 2, 3]]
    이렇게 1차원 리스트인 Vector 가 복수개 모인 2차원의 묶음을 Matrix 라고 합니다.

    이러한 Matrix 가 또 묶이면 마땅히 부를만한 말을 만드는 것도 번거로운 일입니다.
    그래서 여기서부터 차원이 늘어나건 줄어들건 모두, n 차원 Tensor 라고 부릅니다.

    정확히는, 벡터도 텐서고, 매트릭스도 텐서에 속합니다. 즉, 데이터 묶음은 모두 텐서입니다.
...
def main():
    ...
    - 딥러닝 모델을 만들 때, 텐서를 다룰 때에는 torch 의 tensor 객체를 사용해야 합니다.
        앞서 넘파이로 다루는 텐서와는 달리, 
        딥러닝에 필요한 여러 기능을 제공해주고, GPU 연산까지 가능하게 해주는 PyTorch 의 기본 단위입니다.
    """
    # torch 텐서 생성
    print("torch 텐서 생성")
    # 출력 : tensor([1, 2, 3])
