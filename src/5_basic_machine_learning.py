...
def main():
    ...
    [머신러닝 알고리즘 설명]
    - 현재 인공지능 분야에서 가장 강력한 성능을 보장하는 알고리즘은 딥러닝입니다. 그 중에서도 LLM 이죠.
        지금 시점에 모두가 LLM 에 열광을 하는 이때에도 저는 바로 LLM 부터 시작하지는 않을 것입니다.
        
        데이터 분석 관련 책들을 여럿 읽어보아도 LLM 이 강력하니 무조건 LLM 하나만 하면 된다고 말하는 책은 하나도 보지 못했습니다.
        이유는, 검증된 머신러닝 알고리즘이나 그 근간이 되는 수학 등의 지식들은 전문성을 지닌 학자들이 인생의 일부를 바쳐가며 정제하여 발표한 기법이며,
        특정 문제, 특정 데이터, 특정 상황에 따라 각자 최적화된 답을 내어 주도록 완성되었기에, 어느 하나 중요하지 않고 유용하지 않은 것이 없기 때문입니다.
        
        이렇게 만들어진 알고리즘은 잘 만들어진 망치나 톱과 같은 도구라고 생각하면 됩니다.
        그리고 LLM, 딥러닝과 같은 알고리즘은 도구가 아니라 인간을 인공적으로 만들어내는 것을 목표로 하여 만들어 졌습니다.
        
        이렇게 목적도 다르고 원리도 다르므로 그 쓰임새 역시 다르기에 설령 인공지능이 완성되었다고 해도 머신러닝 자체가 사라지는 일은 없을 것입니다.
        그렇기에 저는 제가 본 책들의 의견을 따라서 제가 할 수 있는 만큼 보다 근본적인 지식들 역시 파고들려는 것입니다.
        
        결국 딥러닝이 인공지능을 완성한다고 하면, 그 인공지능도 역시 인간처럼 도구를 사용할 것이고, 즉 머신러닝 알고리즘을 사용할 것입니다.
        
        그리고 그 인공지능을 다스리는 것은 결국 살아있는 인간이 되어야만 한다고 저는 생각합니다.
        그렇기에 저는 그 천재적으로 똑똑한 인공지능을 다스리거나, 이용하거나 공존하기 위해서 그러한 지식들을 이해하는 것이 필요하다고 생각합니다.
        생각해보세요. 세상은 똑똑하고 우수한 사람들의 손에 의해서만 다스려지는 것이 아닙니다.
        제갈량도 유비 아래에 있을 수 밖에 없었고, 그래야만 했던 것처럼, 인공지능 역시 결국 그 목줄은 사람이 쥐게 될 것인데, 
        우리가 해야할 일은 인공지능과 지식 경쟁을 하고 기계와 육체적 힘을 겨루는 것이 아니라, 
        지혜롭고 이치에 맞고 대의에 맞게 그러한 능력을 이용하는 것이 중요하다고 저는 생각합니다.
    """
